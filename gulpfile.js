...
gulp.task('watch', ['browserSync', 'sass', 'less'], function(){
  ...
  gulp.watch('app/less/**/*.less', ['less']); // Watches for changes on these files and runs all the tasks in the array on the second parameter
...
gulp.task('build', function (callback) {
  ...
  runSequence('clean:dist', ['sass', 'less', 'useref', 'images', 'fonts'], callback);
});

// To run default we just need to type gulp
gulp.task('default', function (callback) {
  runSequence(['sass', 'less', 'browserSync', 'watch'], callback);
});
